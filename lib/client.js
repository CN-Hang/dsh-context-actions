  ...
  factory: (require) => {
    ...
    const DICTS = {
      zh: {
        ...
        'card.mechanicalHint': '脚本节选：零模型调用。选「模型总结」后可配置总结模型与提示词',
        'field.prompt': '自定义提示词',
      ...
      en: {
        ...
        'card.mechanicalHint': 'Script digest: no model call. Pick "Model summary" to configure the model and prompt',
    ...
    function apply(ctx) {
      ...
      ctx.inject(['sessions', 'remote', 'remote.commands', 'remote.session', 'workspaces'], (scope) => {
        let timer = 0;
        const schedule = () => {
          if (timer !== 0) return;
          timer = setTimeout(() => {
        ...
        async function runHandover() {
          ...
          try {
            ...
            const fellBack = typeof outcome.text === 'string' && outcome.text.includes('回退');
            ...
            const cwd =
              current.summary !== undefined && typeof current.summary.cwd === 'string' && current.summary.cwd !== ''
                ? current.summary.cwd
                : undefined;
            const workspaceId = workspaceIdOf(scope.workspaces, current.id);
            const createOptions = {};
            if (workspaceId !== undefined) createOptions.workspaceId = workspaceId;
            if (cwd !== undefined) createOptions.cwd = cwd;
            const created = await scope.sessions.create(createOptions);
            ...
            const text = promptText(target, fellBack ? 'mechanical' : described.mode);
            let handle;
            try {
    ...
    /**
     * 当前会话所属的 Workspace id；没有归属（未分组）时返回 undefined。
     * dsh 侧边栏按 WorkspaceView.sessionIds 归组，所以创建新会话时要带上这个 id。
     */
    function workspaceIdOf(workspaces, sessionId) {
      try {
        const snapshot = workspaces === undefined || workspaces === null ? undefined : workspaces.list.getSnapshot();
        const items = snapshot !== undefined && Array.isArray(snapshot.items) ? snapshot.items : [];
        const hit = items.find(
          (item) => item !== undefined && Array.isArray(item.sessionIds) && item.sessionIds.includes(sessionId),
        );
        return hit === undefined ? undefined : hit.workspaceId;
      } catch (error) {
        console.warn('[dsh-context-actions] 读取 workspace 归属失败', error);
        return undefined;
      }
    }

    /** 发给新会话的首条消息：脚本交接先让用户确认下一步。 */
    function promptText(target, mode) {
      if (mode === 'mechanical') {
        return [
          '你正在接手上一会话的工作。',
          '',
          ...
          '读完后先不要动手改任何东西，先问我接下来想怎么做，并给我这两个选项：',
          '1. 由你根据交接文档生成一份「下一步计划」给我确认——你只输出计划，不改文件、不执行操作；',
          '2. 由我手动输入接下来的计划，你再按我说的做。',
          '',
          '等我做完选择并确认后，再开始执行。',
        ].join('\n');
      }
    ...
    function HandoverSettingsCard(props) {
      ...
      // 只有「模型总结」才显示模型/提示词；脚本节选时卡片里只留生成方式。
    ...
    exports.__test = { findContextPanel, promptText, workspaceIdOf };
