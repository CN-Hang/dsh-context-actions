...
const DEFAULT_SUMMARY_PROMPT = [
  ...
  '请把它整理成一份简洁的交接文档：只写接手必需的信息，目标 1-2 页，宁短勿虚；不要为了凑长度复述工具输出。交给一个完全没有这段记忆的新会话，让对方读完就能接着干活。',
  '',
  ...
  '',
  '## 待办与下一步',
  '- 给出接手后建议的下一步计划：按优先级列 1-3 条，写成可执行的小步骤，供用户确认。',
  '- 已知的坑与注意事项。',
  '',
  '## 风险与未解决的问题',
  '- 不确定、未验证、可能有副作用的地方。',
  ...
  '- 篇幅尽量短：目标 1-2 页；信息不足就写「未知」，不要展开猜测。',
...
/** 以下策略固定，刻意不做成配置项（保持卡片只有 4 个字段）。 */
const SUMMARY_TIMEOUT_MS = 180000
/** 不再提供「输出上限」配置；给模型输出预留的固定 token 额度。 */
...
function describeGeneration(built) {
  if (built.summary !== undefined) return built.summary.truncated ? '模型总结，输出被提供方上限截断' : '模型总结'
...
async function buildDocument(ctx, agent, plan) {
  ...
  if (plan.mode === 'llm') {
    try {
      fed = buildTranscript(meta, messages, resolveInputBudget(session))
 ...
 */
/**
 * 计算喂给模型的字符预算：用该会话最近一次请求的上下文窗口，扣掉固定输出预留与余量后换算字符（保守估算）。
 * @param session - 当前会话（可能为 undefined）。
 * @returns 字符预算。
 */
function resolveInputBudget(session) {
  ...
  const usableTokens = Math.max(2048, window - SUMMARY_OUTPUT_RESERVE_TOKENS - 1024)
...
function render(meta, gathered, files, commands, now, extra) {
  ...
  if (summary !== undefined) {
    ...
    tail.push('3. 按「待办与下一步」里的计划继续推进。')
  } else {
    tail.push('1. 先通读本文档；如需细节，可按上表路径解压原始日志检索。')
    tail.push('2. 先不要动手：请用户选择「由你根据文档生成下一步计划供其确认」或「由用户手动输入下一步计划」。')
    tail.push('3. 用户确认计划后，再开始执行。')
  }
  tail.push('')
